...
const SearchIntent_Handler = {
  ...
  async handle(handlerInput) {
    ...
    sessionAttributes.locale = locale;
    ...
    let results = searchResult.length;
    console.log(`[INFO] # of results: ${results}`);

    let say = '';
    if (results > 1) {
      say = `I have found ${results} results for ${titleSlot}. `;
    } else {
      say `I have found only one result for ${titleSlot}`;
    }
    ...
    // sessionAttributes.searchResult = searchResult;
...
const ServiceIntent_Handler = {
  canHandle(handlerInput) {
    const request = handlerInput.requestEnvelope.request;
    return request.type === 'IntentRequest' && request.intent.name === 'ServiceIntent';
  },
  async handle(handlerInput) {
    const { requestEnvelope, responseBuilder, attributesManager } = handlerInput;
    ...
    // let providerId = JSON.stringify(sessionAttributes.topResult.offers[0], null, 4)

    let providerId = '';

    for (i = 0; i < sessionAttributes.topResult.offers.length; i++) {
      let offer = sessionAttributes.topResult.offers[i];
      if (offer !== 'cinema') {
        providerId = offer.provider_id;
      }
    }
    console.log(`[INFO] providerId: ${providerId}`);

    // let jw = sessionAttributes.jw;
    ...
    console.log(`[INFO] providerName: ${providerName}`);

    let say = `It's on ${providerName}`;
...
const LaunchRequest_Handler = {
  ...
  handle(handlerInput) {
    ...
    //       const { favourites } = attributes;

    //       let speechText = `Welcome to ${invocationName}, your first stop to find out where you can stream that movie or tv show you love. `;
    //       let reprompt = '';
          
    //       if (typeof favourites === 'undefined') {
    //         // first run
    //         speechText += 'You have not set up your favorite streaming services yet. Do you want to do it now? ';
...
function startSkill(handlerInput) {
  return new Promise((resolve, reject) => {
    handlerInput.attributesManager.getPersistentAttributes()
      .then((attributes) => {
        const { responseBuilder } = handlerInput;
        let speechText = `Welcome to ${skillName}, your one stop for your streaming needs.`
        let reprompt = '';
